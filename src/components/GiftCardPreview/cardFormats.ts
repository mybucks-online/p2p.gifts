...
import type { CardFormatConfig, CardFormatId } from "@p2p-gifts/types/card";

export const CARD_FORMATS: Record<CardFormatId, CardFormatConfig> = {
  default: {
    label: "Classic",
...
export function getCardFormatConfig(formatId?: string): CardFormatConfig {
  return (
    CARD_FORMATS[formatId as CardFormatId] ??
    CARD_FORMATS[CARD_FORMAT_ID_DEFAULT]
  );
}

interface ParsedCardStyle {
  format: string;
  theme: string | null;
  templateId: string | null;
}

export function parseCardStyle(styleId: string): ParsedCardStyle {
  ...
  if (separator > 0) {
    const format = styleId.slice(0, separator) as CardFormatId;
